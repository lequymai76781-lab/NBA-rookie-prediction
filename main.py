...
rookie_df = None
veteran_df = None
career_debug_df = None

stage1_special_thresholds = {
    1: 0.28,
    2: 0.26,
    3: 0.24,
}
...
def detect_artifact_dir() -> Tuple[str, str]:
    candidate_dirs = [
        BASE_DIR,
        os.path.join(BASE_DIR, "模型文件"),
        os.path.join(BASE_DIR, "model_outputs"),
    ...
    for d in candidate_dirs:
        if not os.path.isdir(d):
            continue
        if os.path.exists(os.path.join(d, "feature_cols.pkl")):
            artifact_dir = d
            break

    if artifact_dir is None:
        raise FileNotFoundError("未找到包含 feature_cols.pkl 的模型输出目录")

    plots_dir_candidates = [
        os.path.join(artifact_dir, "plots"),
        os.path.join(BASE_DIR, "plots"),
    ]
    plots_dir = first_existing_path(plots_dir_candidates)
    if plots_dir is None:
        plots_dir = os.path.join(artifact_dir, "plots")
        os.makedirs(plots_dir, exist_ok=True)

    return artifact_dir, plots_dir
...
def find_csv(candidates: List[str]) -> Optional[str]:
...
def build_core_strength_weakness(row: Dict[str, Any]) -> Tuple[str, str]:
    ...
    if safe_float(row.get("steals_per36")) >= 1.5:
        strengths.append("外线干扰")
    ...
    if safe_float(row.get("usage_pct")) >= 0.28 and safe_float(row.get("true_shooting_pct")) < 0.53:
        weaknesses.append("高占有低效率")
...
def build_rule_secondary_label(row: Dict[str, Any]) -> str:
    ...
    if pts36 >= 20 and ts >= 0.55 and usg >= 0.25:
        return "高效得分核心"
    if usg >= 0.28 and ts < 0.52 and pts36 >= 18:
        return "高使用率低效"
    if pts36 >= 18 and ts < 0.52 and usg >= 0.25:
        return "低效得分手"
    if ast36 >= 7 and atr >= 2.5 and pts36 >= 15:
        return "全能组织者"
    if tp >= 0.36 and dws >= 1.5 and pts36 <= 15:
        return "3D角色球员"
    if blk36 >= 1.5 and reb36 >= 10 and pts36 <= 15:
        return "内线护框型"
    if tp >= 0.35 and blk36 >= 1.0 and pts36 >= 12:
        return "空间内线"
    if stl36 >= 1.5 and dws >= 2.0 and pts36 <= 12:
        return "防守尖兵"
...
def predict_primary_two_stage(row: pd.Series) -> Dict[str, Any]:
    ...
    X_stage1 = preprocess_for_model(
        row_df,
        primary_stage1_imputers[stage1_window],
        primary_stage1_scalers[stage1_window]
    )
    ...
    if stage1_label == "特殊轨迹型" and primary_special_models:
        ...
        X_special = preprocess_for_model(
            row_df,
            primary_special_imputers[special_window],
            primary_special_scalers[special_window]
        )
...
async def lifespan(app: FastAPI):
    global ARTIFACT_DIR, PLOTS_DIR
    global feature_cols
    global primary_stage1_models, primary_stage1_imputers, primary_stage1_scalers, primary_stage1_label_encoders
    global primary_special_models, primary_special_imputers, primary_special_scalers, primary_special_label_encoders
    global rookie_df, veteran_df, career_debug_df

    try:
        print("=" * 60)
        print("⏳ 正在加载最终版模型与数据...")

        ARTIFACT_DIR, PLOTS_DIR = detect_artifact_dir()
        print(f"✅ 模型目录: {ARTIFACT_DIR}")
        print(f"✅ 图片目录: {PLOTS_DIR}")

        feature_cols = load_pickle(os.path.join(ARTIFACT_DIR, "feature_cols.pkl"))

        primary_stage1_models = load_pickle(os.path.join(ARTIFACT_DIR, "primary_stage1_models_by_window.pkl"))
        primary_stage1_imputers = load_pickle(os.path.join(ARTIFACT_DIR, "primary_stage1_imputers_by_window.pkl"))
        primary_stage1_scalers = load_pickle(os.path.join(ARTIFACT_DIR, "primary_stage1_scalers_by_window.pkl"))
        primary_stage1_label_encoders = load_pickle(os.path.join(ARTIFACT_DIR, "primary_stage1_label_encoders_by_window.pkl"))

        primary_special_models = load_pickle(os.path.join(ARTIFACT_DIR, "primary_special_models_by_window.pkl"))
        primary_special_imputers = load_pickle(os.path.join(ARTIFACT_DIR, "primary_special_imputers_by_window.pkl"))
        primary_special_scalers = load_pickle(os.path.join(ARTIFACT_DIR, "primary_special_scalers_by_window.pkl"))
        primary_special_label_encoders = load_pickle(os.path.join(ARTIFACT_DIR, "primary_special_label_encoders_by_window.pkl"))

        rookie_csv = find_csv([
            os.path.join(ARTIFACT_DIR, "current_rookie_samples.csv"),
            os.path.join(BASE_DIR, "current_rookie_samples.csv"),
            os.path.join(ARTIFACT_DIR, "predict_samples.csv"),
            os.path.join(BASE_DIR, "predict_samples.csv"),
        ])
        if rookie_csv is None:
            raise FileNotFoundError("未找到 current_rookie_samples.csv / predict_samples.csv")

        veteran_csv = find_csv([
            os.path.join(ARTIFACT_DIR, "historical_training_samples.csv"),
            os.path.join(BASE_DIR, "historical_training_samples.csv"),
        ])
        if veteran_csv is None:
            raise FileNotFoundError("未找到 historical_training_samples.csv")

        career_debug_csv = find_csv([
            os.path.join(ARTIFACT_DIR, "career_primary_debug.csv"),
            os.path.join(BASE_DIR, "career_primary_debug.csv"),
        ])

        rookie_df = pd.read_csv(rookie_csv).fillna(np.nan)
        veteran_df = pd.read_csv(veteran_csv).fillna(np.nan)
        career_debug_df = pd.read_csv(career_debug_csv).fillna(np.nan) if career_debug_csv else pd.DataFrame()
...
async def root():
    if not os.path.exists(INDEX_HTML_PATH):
        raise HTTPException(status_code=404, detail="前端页面 index.html 不存在")
...
async def get_plot(filename: str):
    if not PLOTS_DIR:
        raise HTTPException(status_code=404, detail="图片目录未初始化")
...
async def get_feature_importance():
    if ARTIFACT_DIR is None:
        raise HTTPException(status_code=500, detail="模型目录未初始化")

    candidates = [
        os.path.join(ARTIFACT_DIR, "shap_outputs", "primary_stage1_window_3_global_shap_importance.csv"),
        os.path.join(ARTIFACT_DIR, "primary_stage1_window_3_global_shap_importance.csv"),
    ]
    fp = first_existing_path(candidates)
    if fp is None:
